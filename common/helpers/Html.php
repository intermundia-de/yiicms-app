...
class Html
{
     ...
     * @author Zura Sekhniashvili <zurasekhniashvili@gmail.com>
     */
    public static function picture(BaseModel $model, $attribute, $options = [])
    {
        /** @var FileManagerItem[] $images */
        $images = $model->activeTranslation->{$attribute};
        $imgSources = [];
        $altText = isset($model->activeTranslation->alt_text) ? $model->activeTranslation->alt_text : $model->activeTranslation->getTitle();
        if ($images) {
